...
Result parse_statement(const char* contents, Token** token, VarList* globals,
                       FunctionList* functions, const TokenType until, AstNode* node)
{
    PtrList values;
...
Result parse_value(const char* contents, Token** token, VarList* globals,
                   FunctionList* functions, AstNode* node)
{
    node->token = *token;
    ...
    case token_identifier:
        ...
        case token_opening_paren:
            ...
            if (indexof_tok == -1) return failure(*token, "unknown funciton");
...
Result parse_args(const char* contents, Token** token, Function* function, VarList* vars, FunctionList* functions,
                  AstNode* inner)
{
