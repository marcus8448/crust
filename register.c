...
void stackframe_init(StackFrame* frame, const StackFrame *parent)
{
    ...
    for (int i = 0; i < 16; i++)
    {
        if (parent != NULL)
        {
            frame->registers[i] = parent->registers[i];
        } else
        {
            frame->registers[i].name = NULL;
            frame->registers[i].type.kind = 0;
            frame->registers[i].type.inner = NULL;
        }
    }
    frame->curOffset = 0;
}
...
void stackframe_claim(StackFrame* frame, Variable variable)
{
    for (int i = 0; i < 16; ++i)
    {
        if (i == rbx || i == rsp) continue;
        if (!frame->active[i])
        {
            frame->registers[i] = variable;
        }
    }
}
